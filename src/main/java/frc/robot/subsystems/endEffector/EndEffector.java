...
public class EndEffector extends SubsystemBase {
    ...
    public boolean outtaking = false; //If true, the End Effector is currently outtaking, if false, it is not.
    ...
    public EndEffector() {
        ...
        //Intialize motors
        m_endEffectorOne = new SparkMax(EndEffectorConstants.endEffectorMotorOne_Port , MotorType.kBrushless);
        m_endEffectorTwo = new SparkMax(EndEffectorConstants.endEffectorMotorTwo_Port , MotorType.kBrushless);
        m_algeDescorer = new SparkMax(EndEffectorConstants.algeDescorerMotor_Port, MotorType.kBrushless);
        ...
        m_endEffectorOne.configure(
            m_endEffectorOne_config, ResetMode.kNoResetSafeParameters, PersistMode.kNoPersistParameters);

        m_endEffectorTwo.configure(
            m_endEffectorTwo_config, ResetMode.kNoResetSafeParameters, PersistMode.kNoPersistParameters);


        //Push config onto the alge Descorer
        m_algeDescorer.configure(m_algeDescorer_config, ResetMode.kNoResetSafeParameters, PersistMode.kNoPersistParameters);
    ...
    public void periodicMain() {
        ...
        //Monitor and control the alge descorer system
        if (runAlgeDescorer == true) {
            runAlgeDescorer(); //Run the alge descorer.
        } else {
            cutAlgeDescorer(); //Cut the alge descorer.
        }
    ...
    public void outakeControl() {
        if (outtakeLimitSwitch.get()) {
            m_endEffectorOne.set(-EndEffectorConstants.endEffectorSpeed); //If the outtake switch is actuated, spin the motors.
            m_endEffectorTwo.set(EndEffectorConstants.endEffectorSpeed);
        } else if (outtakeLimitSwitch.get() == false) { //If the outake switch is off, cut the motors and set the outtaking status back to false.
    ...
    //If the intake limit switch is activated, intake until the outtake limit switch is down and the intake is not.
    public void intakeControl() {
        ...
        if (intakeLimitSwitch.get()) {
            ...
            framesSinceIntakeLimitSwitch = 0; //Since the intake limit switch was pressed, set the frames since last press to zero.
        } else if (framesSinceIntakeLimitSwitch == 1) {
            m_endEffectorOne.set(0.0); //Set the end effector motors to off if its been 1 frame since the limit switch was pressed (limit switch is pressed)
            m_endEffectorTwo.set(0.0);
        }


        framesSinceIntakeLimitSwitch++; //Increase the frames that have passed since the intake limit switch was actuated.
    }
